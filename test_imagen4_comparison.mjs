...
// Exact same approved baseline prompt from test_flux_final_baseline.mjs
...
async function testModel(token, modelId, outputPath, modelLabel) {
  ...
  // Copy to artifacts for display in chat
...
async function main() {
  try {
    ...
    console.log("Got JWT. Starting image tests...");

    // Test 1: google/imagen-4 (standard)
    await testModel(
      token,
      'google/imagen-4',
      'public/assets/keeper_imagen4_test.jpg',
      'Imagen 4 Standard'
    );

    // Test 2: google/imagen-4-ultra (highest quality)
    await testModel(
      token,
      'google/imagen-4-ultra',
      'public/assets/keeper_imagen4_ultra_test.jpg',
      'Imagen 4 Ultra'
    );

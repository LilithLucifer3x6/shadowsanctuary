...
async function main() {
  ...
  try {
    ...
    // --- GRIMOIRE (COMMUNE) ---
    console.log("Clicking 'The Grimoire' tab...");
    await page.click('text="The Grimoire"');
    await page.waitForTimeout(1000);
    
    console.log("Clicking Commune...");
    await page.click('text="Commune"');
    
    await page.waitForSelector('text="Deliver unto the Keeper"', { timeout: 5000 });
    console.log("Capturing Commune Modal screenshot...");
    await page.screenshot({ path: path.join('public', 'assets', 'avatar-tests', 'commune_modal_fixed.png') });
    ...
    console.log("SUCCESS: Captured screenshots.");
  } catch (err) {

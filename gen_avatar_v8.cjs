...
IDENTITY & BODY LOCK: Deep umber skin, explicitly 265 lbs, very heavy and curvy with thick thighs/bust, true waist. ABSOLUTELY NO FRONT BANGS (no hair hanging down in face). Pure black micro-locs. 
...
FOOTWEAR SHAPE/COLOR CORRECTION: Use the second reference image ONLY for the SHAPE (Okobo-style traditional tall sloping wooden block). DO NOT copy its red color or floral design. The shoes must have a solid BLACK wooden platform and CRIMSON straps. If there are any designs on the black platform, they MUST match the silver crane and cherry blossom embroidery of her kimono. Add ankle bracelets made of moons, stars, and forest witch shapes. Retain pretty toe rings. Extremely tall.
...
ART STYLE: Painterly/illustrated digital art matching the character reference exactly. No photorealism. No quarter on the floor.`;
        ...
        if (data.status === 'succeeded' && data.output) {
            ...
            const outPath = `docs/proofs/avatar-v8-${viewName}.png`;
...
async function combineImages() {
    ...
    <html>
        <body style="margin:0; padding:0; background: #1a1a1a; display:flex; justify-content:center; align-items:center;">
            <div style="display:flex; gap: 20px; padding: 20px;">
                <img src="file://${__dirname.replace(/\\/g, '/')}/docs/proofs/avatar-v8-front.png" style="height: 1080px; width: auto;" />
                <img src="file://${__dirname.replace(/\\/g, '/')}/docs/proofs/avatar-v8-back.png" style="height: 1080px; width: auto;" />
            </div>
        </body>
    </html>`;
    ...
    await page.screenshot({ path: 'docs/proofs/avatar-anchor-v8.png', fullPage: true });
    await browser.close();
    fs.unlinkSync('temp_sheet.html');
    console.log('Saved combined sheet to docs/proofs/avatar-anchor-v8.png');
}

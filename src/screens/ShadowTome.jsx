...
export default function ShadowTome({ pose, isEnabled }) {
  ...
  const handleBatchPhoto = async (e) => {
    ...
    // Simulate upload - we create local blob URLs for now to match Rootwork pattern
  ...
  const handleSave = async () => {
    ...
    if (entryText || moodsArray.length > 0 || pendingPhotos.length > 0) {
      const currentText = entryText;
      const currentMoods = moodsArray;
      const currentPhotos = pendingPhotos.map(p => p.url);
          ...
          <div className="card" style={{ padding: '1.5rem' }}>
            ...
            <div className="field" style={{ marginTop: '2.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                <label style={{ margin: 0 }}>The Parchment Inscription</label>
                <div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', overflowX: 'auto', paddingBottom: '0.5rem' }}>
                  {pendingPhotos.map((p, i) => (
                    <div key={i} style={{ position: 'relative', flexShrink: 0 }}>
                      <img src={p.url} style={{ width: '80px', height: '80px', objectFit: 'cover', borderRadius: '4px', border: '1px solid var(--border)' }} />
                      <button className="btn sm" style={{ position: 'absolute', top: -5, right: -5, padding: '0.2rem', background: 'var(--bg)', color: 'var(--red)' }} onClick={() => setPendingPhotos(prev => prev.filter((_, idx) => idx !== i))}>&times;</button>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{ textAlign: 'center', marginTop: '2rem' }}>
              <button id="btn-save-tome" className="btn plum" onClick={handleSave} disabled={!entryText && selectedMoods.size === 0 && pendingPhotos.length === 0}>
                Bind the Parchment
              </button>
            </div>
          </div>

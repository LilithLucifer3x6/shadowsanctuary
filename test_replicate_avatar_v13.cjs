...
async function runPrompt(roomName, customScene, attemptNum) {
    try {
        ...
        const jsonOut = `replicate_raw_response_part3_fix_${roomName}.json`;
        ...
        if (data.status === 'succeeded' && data.output) {
            ...
            const outPath = `public/assets/avatar-tests/part3_fix_${roomName}.png`;
...
async function main() {
    const rooms = [
        ...
        { name: 'shadow_tome', scene: 'Shadow Tome reading. The character is seated in a cozy, intimate reading nook, holding an open ancient book. The setting features dark wood, shadows, lit primarily by a single traditional glowing floor lantern. (Painterly illustration, not photorealistic!).' },
